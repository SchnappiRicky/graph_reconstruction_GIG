#!/usr/bin/env python3
"""Universal gene-only renderer for the Figure4-style JSON schema.
...
Design goals:
- Fully automatic layout (no manual SVG editing)
- Deterministic by seed
- Multi-attempt layout with quality scoring and best-attempt selection
- Built-in validation and machine-readable metrics
"""
...
REQUIRED_NODE_FIELDS = ("gene_node_idx", "gene_node_name", "Weight")
REQUIRED_EDGE_FIELDS = ("Actual_From", "Actual_To", "Weight")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Fully automatic gene-only graph renderer for Figure4-style JSON "
            "(gene_node_idx/gene_node_name/Actual_From/Actual_To/Weight)."
        )
    )
    ...
    parser.add_argument("--width", type=int, default=2200, help="Canvas width.")
    parser.add_argument("--height", type=int, default=1300, help="Canvas height.")
...
def validate_and_load_graph(path: Path) -> Tuple[List[Node], List[Edge], Dict[str, float]]:
    ...
    for idx, row in enumerate(payload["nodes"]):
        ...
        try:
            ...
            raise ValueError(f"Node {node_id} has non-numeric Weight: {row['Weight']} ({exc})") from exc

        if not math.isfinite(weight) or weight < 0:
            invalid_node_weight += 1
            weight = 0.0

        nodes.append(Node(node_id=node_id, name=name, weight=weight))
    ...
    for idx, row in enumerate(payload["edges"]):
        ...
        current = dedup.get((a, b))
        if current is None or weight > current:
            dedup[(a, b)] = weight

    for (a, b), w in dedup.items():
        edges.append(Edge(src=a, dst=b, weight=w))
    ...
    quality = {
        ...
        "empty_gene_names_fixed": float(empty_names),
    }
...
def node_layout_radii(nodes: Sequence[Node]) -> Dict[int, float]:
    """Radii used by layout physics only (local units).

    Keep these relatively small so global normalization does not erase
    render-time size differences.
    """
    ...
    for n in nodes:
        ...
        out[n.node_id] = 0.018 + 0.004 * nw
    return out


def node_render_visual_params(nodes: Sequence[Node]) -> Tuple[Dict[int, float], Dict[int, float]]:
    """Render-time radii/font sizes in pixels.

    Goals:
    - Large but not bloated nodes.
    - Keep visible correlation with node weight.
    - Guarantee label text is enclosed by the node border.
    """
    weights = [n.weight for n in nodes]
    w95 = max(percentile(weights, 0.95), 1e-6)
    ...
    for n in nodes:
        ...
        # Balance readability and network openness.
        font_px = 12 + 2.1 * nw
        weight_radius_px = 20 + 7.8 * nw

        # Circle radius must cover text box diagonal.
...
def relax_positions_for_render(
    ...
    """Pixel-space relaxation to support larger node circles/labels.

    This is fully automatic and keeps output deterministic by seed.
    """
    ids = [n.node_id for n in nodes]
    ...
    # Tiny deterministic jitter avoids zero-gradient deadlocks.
    for i in ids:
    ...
    for _ in range(340):
        ...
        for e in edges:
            ...
            # Strongly lengthen edges so links stay clear with large nodes/text.
        ...
        for i in ids:
            x, y = p[i]
            hx, hy = home[i]
            # Keep geometry coherent but avoid pulling back too tightly.
            disp[i][0] += (hx - x) * 0.007
            disp[i][1] += (hy - y) * 0.007
...
def edge_stroke(weight: float, w95: float) -> Tuple[str, float, float]:
...
def node_fill(weight: float, w95: float) -> str:
    norm = clamp(math.sqrt(weight / max(w95, 1e-9)), 0.0, 1.2)
    # blue palette from pale to saturated
...
def build_svg(
    ...
    weights = [n.weight for n in nodes]
    w95 = max(percentile(weights, 0.95), 1e-6)
    hub_threshold = percentile(weights, 0.90)
    ...
    for idx, e in enumerate(sorted(edges, key=lambda x: x.weight)):
        ...
        color, sw, op = edge_stroke(e.weight, w95)
        dx, dy = x2 - x1, y2 - y1
        d = math.hypot(dx, dy) + 1e-6
        px, py = -dy / d, dx / d
    ...
    for n in sorted(nodes, key=lambda x: x.weight):
        ...
        fill = node_fill(n.weight, w95)
        parts.append(f'<circle cx="{x:.2f}" cy="{y:.2f}" r="{r:.2f}" fill="{fill}" stroke="none"/>')
        if n.weight >= hub_threshold:
            parts.append(
    ...
    parts.extend(
        [
            "</g>",
            '<g id="legend">',
            '<circle cx="112" cy="46" r="7" fill="#c7e4f1"/>',
            '<text x="126" y="50" font-family="Helvetica,Arial,sans-serif" font-size="12" fill="#4b5563">Gene</text>',
            '<circle cx="112" cy="68" r="7" fill="none" stroke="#ef7d00" stroke-width="2"/>',
            '<text x="126" y="72" font-family="Helvetica,Arial,sans-serif" font-size="12" fill="#4b5563">Top-weight hub</text>',
            '<path d="M 109,88 Q 123,78 137,88" stroke="#7b7b7b" stroke-width="2" fill="none"/>',
            '<text x="147" y="92" font-family="Helvetica,Arial,sans-serif" font-size="12" fill="#4b5563">Gene-Gene edge</text>',
            "</g>",
            "</svg>",
        ]
    )
...
def run(args: argparse.Namespace) -> int:
    ...
    metrics_payload = {
        ...
        "schema": {
            "node_fields": list(REQUIRED_NODE_FIELDS),
            "edge_fields": list(REQUIRED_EDGE_FIELDS),
        },
        ...
        "timing": {
            ...
            # Includes input validation, all layout attempts, and SVG write.
    ...
    print(
        f"Selected seed: {best.seed} | score={best.score:.3f} | "
        f"run_total={run_total_seconds:.3f}s"
    )
    return 0
