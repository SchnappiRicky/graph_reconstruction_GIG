...
OUTPUT_EDGE_CSV = ROOT / "Figure_5_drug_edges.csv"
OUTPUT_CITATION_CSV = ROOT / "Figure_5_drug_citation_legend.csv"
...
def load_figure5_graph():
    ...
    for raw in data["nodes"]:
        weight = max(float(raw["Weight1"]), float(raw["Weight2"]))
        nodes.append(
            {
                **raw,
                "Weight": weight,
    ...
    pvalue_ids = set()
    for node in nodes:
        t2d_p = float(node["t2ds_no_t2ds_pvalue"])
        pret2d_p = float(node["pret2ds_no_t2ds_pvalue"])
        if t2d_p < 0.05 and pret2d_p < 0.05:
            category = "both"
        elif t2d_p < 0.05:
            category = "t2ds_only"
        elif pret2d_p < 0.05:
            category = "pret2ds_only"
        else:
            category = "none"
        sig_categories[node["gene_node_idx"]] = category
        if category != "none":
            pvalue_ids.add(node["gene_node_idx"])

    return pvalue_ids, sig_categories, nodes, edges


def gene_fill(sig_category):
    return SIG_COLORS.get(sig_category, SIG_COLORS["none"])
...
def render_legend(parts):
    parts.extend(
        [
            '<g id="legend">',
            f'<circle cx="110" cy="42" r="7" fill="{SIG_COLORS["none"]}" stroke="none"/>',
            '<text x="124" y="47" font-family="Helvetica,Arial,sans-serif" font-size="12.5" fill="#4a4a4a">Core genes</text>',
            f'<circle cx="110" cy="63" r="7" fill="{SIG_COLORS["t2ds_only"]}" stroke="none"/>',
            '<text x="124" y="68" font-family="Helvetica,Arial,sans-serif" font-size="12.5" fill="#4a4a4a">T2D-only significant</text>',
            f'<circle cx="110" cy="84" r="7" fill="{SIG_COLORS["pret2ds_only"]}" stroke="none"/>',
            '<text x="124" y="89" font-family="Helvetica,Arial,sans-serif" font-size="12.5" fill="#4a4a4a">preT2D-only significant</text>',
            f'<circle cx="110" cy="105" r="7" fill="{SIG_COLORS["both"]}" stroke="none"/>',
            '<text x="124" y="110" font-family="Helvetica,Arial,sans-serif" font-size="12.5" fill="#4a4a4a">Shared significant genes</text>',
            '<circle cx="110" cy="126" r="7" fill="none" stroke="#ff8f1f" stroke-width="2.5"/>',
            '<text x="124" y="131" font-family="Helvetica,Arial,sans-serif" font-size="12.5" fill="#4a4a4a">Top-weight hub genes</text>',
            '<rect x="108" y="142" width="20" height="12" rx="6" fill="#a8ddb5" stroke="#2f855a" stroke-width="1.5"/>',
            '<text x="138" y="152" font-family="Helvetica,Arial,sans-serif" font-size="12.5" fill="#4a4a4a">Approved drug</text>',
            '<rect x="108" y="160" width="20" height="12" rx="6" fill="#f6d58b" stroke="#c88719" stroke-width="1.5"/>',
            '<text x="138" y="170" font-family="Helvetica,Arial,sans-serif" font-size="12.5" fill="#4a4a4a">Investigational drug</text>',
            '<path d="M 108,188 Q 122,178 136,188" stroke="#a8a8a8" stroke-width="1.8" fill="none"/>',
            '<text x="146" y="193" font-family="Helvetica,Arial,sans-serif" font-size="12.5" fill="#4a4a4a">Gene-Gene</text>',
            '<path d="M 108,209 Q 122,199 136,209" stroke="#111111" stroke-width="3.6" stroke-linecap="round" fill="none"/>',
            '<text x="146" y="214" font-family="Helvetica,Arial,sans-serif" font-size="12.5" fill="#4a4a4a">Important Gene-Gene</text>',
            '<path d="M 108,230 Q 122,220 136,230" stroke="#c88719" stroke-width="2.0" stroke-dasharray="7 5" fill="none"/>',
            '<text x="144" y="235" font-family="Helvetica,Arial,sans-serif" font-size="9.5" fill="#6b6b6b">PMID:12345678</text>',
            '</g>',
        ]
    )
...
def render_svg(nodes, components, sig_categories):
    ...
    for component in components:
        for node in sorted(component["genes"], key=lambda item: item["Weight"]):
            ...
            fill = gene_fill(sig_categories[node["gene_node_idx"]])
...
def main():
    ...
    svg_text, edge_rows, citation_rows = render_svg(nodes, components, sig_categories)
